...
const SubLabel = ({ children }: { children: React.ReactNode }) => (
  <div className="mb-2 font-mono text-[8px] uppercase tracking-[0.2em]" style={{ color: FORTRESS.textDim }}>
    {children}
  </div>
);
...
const StatusPill = ({ children }: { children: React.ReactNode }) => (
  <span
    className="rounded-sm border px-2 py-1 font-mono text-[8px] font-extrabold uppercase tracking-[0.14em]"
...
export const SettingsDialog = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className="max-h-[92vh] max-w-[min(92vw,900px)] overflow-hidden p-0"
      ...
      >
        ...
        >
          <DialogTitle className="font-mono text-[11px] font-extrabold uppercase tracking-[0.2em] text-green-400">
            Terminal Settings
          </DialogTitle>
          <span className="hidden font-mono text-[8px] uppercase tracking-[0.16em] sm:inline" style={{ color: FORTRESS.textFaint }}>
            Synced to account settings
          </span>
        </DialogHeader>

        <div className="grid min-h-[520px] grid-cols-1 md:grid-cols-[190px_1fr]">
          <nav className="flex gap-1.5 overflow-x-auto border-b p-3 md:flex-col md:overflow-visible md:border-b-0 md:border-r" style={{ borderColor: FORTRESS.borderFaint }}>
            {SECTIONS.map(({ id, label, icon: Icon }) => {
              ...
              return (
                <button
                  ...
                  className="flex shrink-0 items-center gap-2 rounded-sm border px-2.5 py-2 text-left font-mono text-[9px] font-bold uppercase tracking-wide transition-colors fortress-focus"
                  style={{
          ...
          <div className="min-w-0 overflow-y-auto p-4">
            {section === 'profile' && (
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-4 rounded-sm border p-4 sm:flex-row sm:items-center" style={{ borderColor: FORTRESS.border, background: FORTRESS.surfaceRaised }}>
                  <div className="grid h-16 w-16 flex-none place-items-center rounded-sm border font-mono text-xl font-extrabold" style={{ borderColor: FORTRESS.borderGreen, background: '#12301F', color: FORTRESS.greenSoft }}>
                    {initials}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-mono text-sm font-bold tracking-wide" style={{ color: FORTRESS.textBright }}>
                      {profile.callSign || 'Operator'}
                    </div>
                    <div className="truncate font-mono text-[10px]" style={{ color: FORTRESS.textDim }}>
                      {profile.email || 'Email hidden'}
                    </div>
                    <Button
                      type="button"
                      onClick={onEditProfile}
                      className="mt-2 h-auto bg-transparent px-3 py-1.5 font-mono text-[8px] font-extrabold uppercase tracking-wide text-green-400 hover:bg-green-500/10"
                ...
                <div className="rounded-sm border p-3" style={{ borderColor: FORTRESS.amberBorder, background: alpha(FORTRESS.amber, 0.04) }}>
                  <div className="flex items-start gap-2">
                    <AlertTriangle className="mt-0.5 h-4 w-4 flex-none text-amber-500" />
                    <div className="font-mono text-[9px] leading-relaxed tracking-wide text-amber-300/80">
                  ...
                  <Button
                    ...
                    className="mt-3 h-auto w-full bg-transparent py-2 font-mono text-[8px] font-extrabold uppercase tracking-[0.15em] text-amber-300 hover:bg-amber-400/10 disabled:opacity-40"
            ...
            {section === 'security' && (
              <div className="flex flex-col gap-2">
                ...
                {security.autoDeleteMessages && (
                  <div className="rounded-sm border p-3" style={{ borderColor: FORTRESS.border, background: FORTRESS.surfaceSunken }}>
                ...
                <div className="rounded-sm border p-3 font-mono text-[9px] leading-relaxed" style={{ borderColor: FORTRESS.borderGreen, background: alpha(FORTRESS.green, 0.06), color: FORTRESS.textDim }}>
                ...
                {notifications.reminderTimerEnabled && (
                  <div className="rounded-sm border p-3" style={{ borderColor: FORTRESS.border, background: FORTRESS.surfaceSunken }}>
                    ...
                    <Input
                      ...
                      className="w-28 rounded-sm border font-mono text-xs"
                      style={{ borderColor: FORTRESS.border, background: FORTRESS.surfaceRaised, color: FORTRESS.text }}
                    />
                  </div>
                )}
                <div className="rounded-sm border p-3 font-mono text-[9px] leading-relaxed" style={{ borderColor: FORTRESS.borderGreen, background: alpha(FORTRESS.green, 0.05), color: FORTRESS.textDim }}>
            ...
            {section === 'calls' && (
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-4 gap-2 text-center font-mono text-[8px] uppercase tracking-wide text-green-400/70">
                  ...
                  <Vibrate className="mx-auto h-4 w-4" />
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ACCENT_SWATCHES.map((color) => (
                      <button
                        ...
                        className="grid h-8 w-11 place-items-center rounded-sm font-mono text-[11px] font-extrabold text-[#06130B] fortress-focus"
                        style={{ background: color, border: `2px solid ${theme.accentColor === color ? FORTRESS.textBright : 'transparent'}` }}
                      >
                        {theme.accentColor === color ? <Check className="h-3.5 w-3.5" /> : null}
                      </button>
                    ))}
                  </div>
                </div>
                    ...
                    {(['rounded', 'square', 'minimal'] as const).map((bubbleStyle) => {
                      ...
                      return (
                        <button
                          ...
                          className="rounded-sm p-3 text-center font-mono fortress-focus"
                          style={{
                        ...
                        >
                          <div
                            className="mx-auto mb-2 h-3.5 w-[70%]"
                            style={{
                          ...
                          />
                          <div className="text-[9px] font-extrabold uppercase tracking-wide" style={{ color: active ? accent : FORTRESS.textDim }}>
                            {bubbleStyle}
                          </div>
                        </button>
                      );
                  ...
                  <div className="flex flex-wrap gap-2">
                    {BG_PRESETS.map((background) => (
                      <button
                        ...
                        className="grid h-9 w-14 place-items-center rounded-sm font-mono text-[11px] fortress-focus"
                        style={{
                          background,
                      ...
                      >
                        {theme.chatBackground === background ? <Check className="h-3.5 w-3.5" /> : null}
                      </button>
                    ))}
                  </div>
                </div>
                    ...
                    {(['mono', 'sans', 'serif'] as const).map((fontStyle) => {
                      ...
                      return (
                        <button
                          ...
                          className="rounded-sm p-3 text-center fortress-focus"
                          style={{
                            background: active ? alpha(accent, 0.08) : FORTRESS.surfaceRaised,
                            border: `1px solid ${active ? accent : FORTRESS.border}`,
                          }}
                        >
                          <div className="text-sm" style={{ fontFamily, color: FORTRESS.text }}>
                            Aa Bb 01
                          </div>
                          <div className="mt-1.5 font-mono text-[8px] font-extrabold uppercase tracking-wide" style={{ color: active ? accent : FORTRESS.textDim }}>
                            {fontStyle}
                          </div>
                        </button>
                      );
            ...
            {section === 'about' && (
              <div className="flex flex-col items-center gap-1.5 py-4 text-center">
                <div className="mb-1.5 grid h-12 w-12 place-items-center rounded-sm border border-green-500 bg-green-500/10 p-3">
                  <ShieldCheck className="h-6 w-6 text-green-400" />
                </div>
                <div className="font-mono text-base font-extrabold tracking-[0.2em]" style={{ color: FORTRESS.textBright }}>
                  SECURECHAT FORTRESS
                </div>
                <div className="font-mono text-[8px] uppercase tracking-[0.2em]" style={{ color: FORTRESS.textFaint }}>
                  Terminal v1.0.0
                </div>
                <div className="mt-2.5 max-w-md rounded-sm border p-3 font-mono text-[9px] leading-relaxed tracking-wide" style={{ borderColor: FORTRESS.border, background: FORTRESS.surfaceRaised, color: FORTRESS.textDim }}>
                ...
                <div className="mt-2 grid gap-1.5 text-left font-mono text-[9px] uppercase tracking-wide" style={{ color: FORTRESS.text }}>
                ...
                <div className="mt-3 font-mono text-[8px] uppercase tracking-[0.16em]" style={{ color: FORTRESS.textFaint }}>
        ...
        <div className="flex items-center justify-between border-t px-4 py-3" style={{ borderColor: FORTRESS.borderFaint }}>
          ...
          <Button
            type="button"
            onClick={onClose}
            className="h-auto rounded-sm px-5 py-2 font-mono text-[9px] font-extrabold uppercase tracking-[0.2em] text-[#06130B]"
