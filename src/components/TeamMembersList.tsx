...
const fetchTeamMembers = async (teamId: string): Promise<TeamMemberWithProfile[]> => {
  ...
  for (const memberDoc of membersSnap.docs) {
    ...
    // Get profile
    let profile = null;
    const profileRef = doc(db, 'profiles', memberData.user_id);
    ...
    members.push({
      ...
      role: memberData.role || 'team_user',
      profiles: profile
    });
  }
...
const updateMemberRole = async ({ memberId, role }: { memberId: string; role: TeamRole }) => {
...
export const TeamMembersList = ({ teamId, teamName, currentUserRole }: TeamMembersListProps) => {
  ...
  const getMemberName = (member: TeamMemberWithProfile) => {
    if (!member.profiles) return 'Unknown User';
    if (member.profiles.firstName && member.profiles.lastName) {
      return `${member.profiles.firstName} ${member.profiles.lastName}`;
    }
    return member.profiles.callSign || 'Unknown User';
  };
