-- =====================================================
-- FORTRESS MESSAGE COMM - FRESH DATABASE SETUP
-- This drops existing tables and recreates everything
-- =====================================================
...
BEGIN
  ...
  WHERE team_invitations.invitation_code = accept_team_invitation.invitation_code
  AND status = 'pending'
  AND expires_at > now();
  ...
  INSERT INTO public.team_members (team_id, user_id, role)
  VALUES (invitation_record.team_id, v_user_id, invitation_record.role);
...
-- =====================================================
-- 9. CONVERSATION NOTIFICATIONS TABLE
-- =====================================================
...
BEGIN
  ...
  AND EXISTS (
    ...
    WHERE cp1.conversation_id = c.id AND cp1.user_id = current_user_id AND cp1.is_active = true
  )
  AND EXISTS (
    SELECT 1 FROM public.conversation_participants cp2
    WHERE cp2.conversation_id = c.id AND cp2.user_id = other_user_id AND cp2.is_active = true
  )
  AND (SELECT COUNT(*) FROM public.conversation_participants cp WHERE cp.conversation_id = c.id AND cp.is_active = true) = 2;
...
BEGIN
  RETURN QUERY
  SELECT p.id, p.username, p.full_name, p.user_number, p.avatar_url
  FROM public.profiles p
  WHERE p.id != auth.uid()
  AND (
    LOWER(p.username) LIKE LOWER('%' || search_term || '%') OR
    LOWER(p.full_name) LIKE LOWER('%' || search_term || '%') OR
    LOWER(p.call_sign) LIKE LOWER('%' || search_term || '%') OR
...
CREATE POLICY "Users can view ghost sessions they created or are members of"
  ...
  USING (
    ...
    EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_sessions.id AND user_id = auth.uid() AND is_active = true)
  );

CREATE POLICY "Team members can create ghost sessions"
  ON public.ghost_sessions FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.team_members WHERE team_id = ghost_sessions.team_id AND user_id = auth.uid()));
...
-- =====================================================
-- 12. GHOST SESSION MEMBERS TABLE
-- =====================================================
...
CREATE POLICY "Session members can view membership"
  ON public.ghost_session_members FOR SELECT
  USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.ghost_sessions WHERE id = ghost_session_members.session_id AND created_by = auth.uid()));

CREATE POLICY "Session creators can manage members"
  ON public.ghost_session_members FOR ALL
  USING (EXISTS (SELECT 1 FROM public.ghost_sessions WHERE id = ghost_session_members.session_id AND created_by = auth.uid()));

CREATE POLICY "Users can join sessions"
  ON public.ghost_session_members FOR INSERT
  WITH CHECK (user_id = auth.uid());

-- =====================================================
-- 13. GHOST MESSAGES TABLE
-- =====================================================
...
CREATE POLICY "Session members can view messages"
  ...
  USING (EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_messages.session_id AND user_id = auth.uid() AND is_active = true));

CREATE POLICY "Session members can send messages"
  ON public.ghost_messages FOR INSERT
  WITH CHECK (sender_id = auth.uid() AND EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_messages.session_id AND user_id = auth.uid() AND is_active = true));

-- =====================================================
-- 14. GHOST SESSION FUNCTIONS
-- =====================================================
...
DECLARE
  ...
  max_allowed INTEGER;
BEGIN
  SELECT COUNT(gsm.user_id), gs.max_members INTO current_members, max_allowed
