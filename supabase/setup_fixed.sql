-- =====================================================
-- FORTRESS MESSAGE COMM - FIXED DATABASE SETUP
...
-- =====================================================
-- PART 1: CREATE ALL TYPES AND TABLES
-- =====================================================
...
-- Teams policies
CREATE POLICY "Authenticated users can create teams." ON public.teams FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());
CREATE POLICY "Team members can view their teams." ON public.teams FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.team_members WHERE team_members.team_id = teams.id AND team_members.user_id = auth.uid()));

-- Team members policies
CREATE POLICY "Team leads can add members." ON public.team_members FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM public.team_members tm WHERE tm.team_id = team_members.team_id AND tm.user_id = auth.uid() AND tm.role IN ('diamond_in_the_rough', 'team_lead')) OR NOT EXISTS (SELECT 1 FROM public.team_members tm WHERE tm.team_id = team_members.team_id));
CREATE POLICY "Users can see team members." ON public.team_members FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.team_members AS tm WHERE tm.team_id = team_members.team_id AND tm.user_id = auth.uid()));
CREATE POLICY "Users can leave teams." ON public.team_members FOR DELETE TO authenticated USING (user_id = auth.uid());
CREATE POLICY "Team leads can update roles." ON public.team_members FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM public.team_members tm WHERE tm.team_id = team_members.team_id AND tm.user_id = auth.uid() AND tm.role IN ('diamond_in_the_rough', 'team_lead')));

-- Team invitations policies
CREATE POLICY "Team members can view invitations" ON public.team_invitations FOR SELECT USING (EXISTS (SELECT 1 FROM public.team_members WHERE team_id = team_invitations.team_id AND user_id = auth.uid()));
CREATE POLICY "Team leads can create invitations" ON public.team_invitations FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.team_members WHERE team_id = team_invitations.team_id AND user_id = auth.uid() AND role IN ('diamond_in_the_rough', 'team_lead')));
CREATE POLICY "Team leads can update invitations" ON public.team_invitations FOR UPDATE USING (EXISTS (SELECT 1 FROM public.team_members WHERE team_id = team_invitations.team_id AND user_id = auth.uid() AND role IN ('diamond_in_the_rough', 'team_lead')));

-- Conversations policies
CREATE POLICY "Users can view their conversations" ON public.conversations FOR SELECT USING (EXISTS (SELECT 1 FROM public.conversation_participants WHERE conversation_id = conversations.id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Users can update their conversations" ON public.conversations FOR UPDATE USING (EXISTS (SELECT 1 FROM public.conversation_participants WHERE conversation_id = conversations.id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Users can create conversations" ON public.conversations FOR INSERT TO authenticated WITH CHECK (true);

-- Conversation participants policies
CREATE POLICY "Users can view participants" ON public.conversation_participants FOR SELECT USING (EXISTS (SELECT 1 FROM public.conversation_participants cp WHERE cp.conversation_id = conversation_participants.conversation_id AND cp.user_id = auth.uid() AND cp.is_active = true));
...
CREATE POLICY "Users can view messages" ON public.direct_messages FOR SELECT USING (EXISTS (SELECT 1 FROM public.conversation_participants WHERE conversation_id = direct_messages.conversation_id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Users can send messages" ON public.direct_messages FOR INSERT WITH CHECK (sender_id = auth.uid() AND EXISTS (SELECT 1 FROM public.conversation_participants WHERE conversation_id = direct_messages.conversation_id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Users can update own messages" ON public.direct_messages FOR UPDATE USING (sender_id = auth.uid());

-- Conversation notifications policies
CREATE POLICY "Users can view own notifications" ON public.conversation_notifications FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can update own notifications" ON public.conversation_notifications FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "System can insert notifications" ON public.conversation_notifications FOR INSERT WITH CHECK (true);

-- Ghost sessions policies
CREATE POLICY "Users can view ghost sessions" ON public.ghost_sessions FOR SELECT USING (created_by = auth.uid() OR EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_sessions.id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Team members can create ghost sessions" ON public.ghost_sessions FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.team_members WHERE team_id = ghost_sessions.team_id AND user_id = auth.uid()));
CREATE POLICY "Creators can update ghost sessions" ON public.ghost_sessions FOR UPDATE USING (created_by = auth.uid());

-- Ghost session members policies
CREATE POLICY "Users can view session membership" ON public.ghost_session_members FOR SELECT USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.ghost_sessions WHERE id = ghost_session_members.session_id AND created_by = auth.uid()));
CREATE POLICY "Creators can manage members" ON public.ghost_session_members FOR ALL USING (EXISTS (SELECT 1 FROM public.ghost_sessions WHERE id = ghost_session_members.session_id AND created_by = auth.uid()));
CREATE POLICY "Users can join sessions" ON public.ghost_session_members FOR INSERT WITH CHECK (user_id = auth.uid());

-- Ghost messages policies
CREATE POLICY "Session members can view ghost messages" ON public.ghost_messages FOR SELECT USING (EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_messages.session_id AND user_id = auth.uid() AND is_active = true));
CREATE POLICY "Session members can send ghost messages" ON public.ghost_messages FOR INSERT WITH CHECK (sender_id = auth.uid() AND EXISTS (SELECT 1 FROM public.ghost_session_members WHERE session_id = ghost_messages.session_id AND user_id = auth.uid() AND is_active = true));
...
-- =====================================================
-- PART 4: CREATE ALL FUNCTIONS AND TRIGGERS
-- =====================================================
...
CREATE FUNCTION public.accept_team_invitation(invitation_code text)
...
BEGIN
  ...
  SELECT * INTO invitation_record FROM public.team_invitations WHERE team_invitations.invitation_code = accept_team_invitation.invitation_code AND status = 'pending' AND expires_at > now();
  IF invitation_record.id IS NULL THEN RETURN json_build_object('success', false, 'message', 'Invalid or expired invitation'); END IF;
  IF EXISTS (SELECT 1 FROM public.team_members WHERE team_id = invitation_record.team_id AND user_id = v_user_id) THEN RETURN json_build_object('success', false, 'message', 'User is already a team member'); END IF;
...
BEGIN
  ...
  IF v_conversation_id IS NULL THEN
    ...
    INSERT INTO public.conversation_notifications (user_id, conversation_id, type, message_preview) VALUES (other_user_id, v_conversation_id, 'new_chat', 'Started a new conversation with you');
  END IF;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.team_members WHERE team_id = p_team_id AND user_id = auth.uid()) THEN RAISE EXCEPTION 'User is not a member of this team'; END IF;
...
BEGIN
  ...
  IF max_allowed IS NULL THEN RAISE EXCEPTION 'Ghost session not found or inactive'; END IF;
  IF current_members >= max_allowed THEN RAISE EXCEPTION 'Ghost session is full'; END IF;
...
CREATE POLICY "Authenticated users can upload attachments" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'chat_attachments');
CREATE POLICY "Authenticated users can view attachments" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'chat_attachments');
CREATE POLICY "Users can update their own attachments" ON storage.objects FOR UPDATE TO authenticated USING (auth.uid() = owner);
CREATE POLICY "Users can delete their own attachments" ON storage.objects FOR DELETE TO authenticated USING (auth.uid() = owner);
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'conversation_notifications') THEN
